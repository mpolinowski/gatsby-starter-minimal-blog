      ...
      {siteConfig.jobs && siteConfig.jobs.map(job => (
        <article key={job.begin.month + job.begin.year} className='timeline__item'>
          <div className="inner">
            ...
            <h2 className='timeline__title'>{job.company} <br /> <small className='timeline__title--small'>{job.occupation}</small> <br /><small className='timeline__date--small'>{job.duration || 'present'}</small></h2>
            <p>{job.description}</p>
          </div>
        </article>
      ))}
