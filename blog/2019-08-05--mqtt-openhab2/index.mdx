...
In the expanded settings we have to set up a transformation for the incoming topic `JSONPATH:$.val`. This will extract the value from the incoming message - `{"val": "1"}` will become `1` - to set the state of the switch to 1 or 0. For the Quality-of-Service `QoS` choose __At-least-once__. Now click on Save to continue.
